...
/* PROTO */
oauth_s_t *
oauth_init(int version, int sig_method, char *oauth_consumer_key, char *oauth_consumer_secret)
{
...
oauth_set_token(oauth_s_t *s, char *oauth_token, char *oauth_token_secret)
{
    ...
    snprintf(s->signing_key, strlen(s->oauth_consumer_secret)+1+
            strlen(oauth_token_secret)+1, "%s&%s", s->oauth_consumer_secret,
            oauth_token_secret);
...
oauth_prepare(oauth_s_t *s)
{
    ...
    if(n != NULL) {
        ...
        snprintf(timestamp, sizeof(timestamp), "%lu", (unsigned long)ts);
	 ...
	 *
        ...
        if(base64_encode((unsigned char *)buffer, &buflen, random, 32) == 0) {
...
/* PROTO */
void
make_param_str(lc_item_t *i, va_list ap)
{
    va_list ap2;
    kv_t *ki = (kv_t *)i;
    char *buf;
...
/* PROTO */
void
make_query_str(kv_t *i, va_list ap)
{
    va_list ap2;
...
/* PROTO */
void
make_oauth_header(kv_t *i, va_list ap)
{
    char *buf, *ekey, *evalue;
    size_t bufsize;
    va_list ap2;
...
oauth_sign(oauth_r_t *oreq)
{
    ...
    final_param_list = lc_list_create((lc_createfn_t)final_list_create,
            (lc_destroyfn_t)final_list_free, (lc_comparefn_t) kv_t_cmp);
    ...
    lc_list_foreach_v(final_param_list, (lc_foreachfn_v_t)make_param_str, buf, sizeof(buf));
    ...
    sha1_hmac((unsigned char *)oreq->state->signing_key, strlen(oreq->state->signing_key),
            (unsigned char *)final_key_buf, strlen(final_key_buf), sha1_out);
    ...
    memset(buf, 0, sizeof(buf));
    lc_list_foreach_v(oreq->oauth_params, (lc_foreachfn_v_t)make_oauth_header,
            buf, sizeof(buf));

    if(oreq->headers == NULL)
        oreq->headers = lc_list_create((lc_createfn_t)param_list_create,
                (lc_destroyfn_t)param_list_destroy,
                (lc_comparefn_t)kv_t_cmp);
