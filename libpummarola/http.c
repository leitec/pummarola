...
{ 
    ...
    if(response->header == NULL)
        response->header = lc_list_create((lc_createfn_t)param_list_create_nodup,
...
/* PROTO */
void
print_oauth_params(kv_t *i, va_list ap)
{
    int *c, m;
    char *ekey, *evalue;
    char *request;
...
/* PROTO */
void
print_extra_params(kv_t *i, va_list ap)
{
    char *request;
    size_t req_size;
    va_list ap2;
...
/* PROTO */
void
print_query_str(kv_t *i, va_list ap)
{
    va_list ap2;
...
send_signed_https_direct(oauth_r_t *oreq, http_response *response)
{
    ...
    *tptr2 = 0;
    ...
    lc_list_foreach_v(oreq->oauth_params, (lc_foreachfn_v_t)print_oauth_params,
            m, &c, request, req_size);
...
#ifdef macintosh
    ssl_set_bio(oreq->ssl, mactcp_recv, &mc,
                     mactcp_send, &mc);
#else
    ssl_set_bio(oreq->ssl, net_recv, &server_fd,
                     net_send, &server_fd);
#endif
