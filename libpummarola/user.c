	    ...
	    for(j = 0; j < tojv->u.object.length; j++) {
		    ...
		    if(strcmp(name, "user") == 0) {
			    for(k = 0; k < ttjv->u.object.length; k++) {
				    char *uv_name = ttjv->u.object.values[k].name;
		    		    json_value *uv_jv = ttjv->u.object.values[k].value;

				    if(strcmp(uv_name, "name") == 0) {
					    tweet_user = uv_jv->u.string.ptr;
				    }
			    }
		    }
